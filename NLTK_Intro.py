...
text = " ".join(open('bert_extracted_text.txt',encoding='utf-8').read().splitlines())#WHole text data from the file
words = word_tokenize(text.lower())

#Let us get the count of unique stop words in words
print('Print of unique stop words: ',len(set([word for word in words if word in spacy_stop_words])))

# For the count of words which are not stopwords
print('Print of unique words which are not stopwords: ',len(set([word for word in words if word not in spacy_stop_words])))

# What does the text look like without any stopwords
print(' '.join([word for word in words if word not in spacy_stop_words]))
